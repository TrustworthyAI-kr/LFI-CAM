...
def main():
    ...
    extraset = dataloader(root='./data', split= 'extra', download=True, transform=transform_train)

    datas = np.concatenate([trainset.data, extraset.data], axis=0)
    labels = np.concatenate([trainset.labels, extraset.labels], axis=0)
    trainset.data = datas
    ...
    with torch.no_grad():
        ...
        for batch_idx, (inputs, targets) in enumerate(testloader):
            ...
            for item_img, item_att in zip(d_inputs, c_att):
                ...
                cv2.imwrite('stock2_1.png', v_img)
                cv2.imwrite('stock2_2.png', resize_att)
                v_img = cv2.imread('stock2_1.png')
                vis_map = cv2.imread('stock2_2.png')
